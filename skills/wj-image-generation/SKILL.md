...
Use the WJ plugin's `generate_image` and `edit_image` tools for explicit WJ requests and visible native-image fallback conditions.
...
4. Retry the same request at most once for a transient timeout or `502`-class upstream failure.
5. Do not retry authentication, authorization, WJ quota, or WJ rate-limit failures. Report the actionable error clearly.
