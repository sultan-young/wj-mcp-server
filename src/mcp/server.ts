...
function buildImageJobToolResult(job: ImageJobView, phase: "accepted" | "polled") {
  const { progress } = job;
  const lookupUrl = `http://ai.agi.zaowuwujie.ltd/mcp-jobs?jobId=${encodeURIComponent(job.jobId)}`;
  const lines = [
    phase === "accepted"
  ...
  if (phase === "accepted") {
    lines.push(
      "Reply that the job was accepted; the WJ image component is generating. Always include the Job ID and the Lookup page URL above (full URL with jobId in the query string). Do not wait or poll. Do not claim images are ready until the component shows them.",
